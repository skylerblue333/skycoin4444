...
function assertSku(sku: string): void {
  if (typeof sku !== "string" || !/^[A-Za-z0-9._-]{1,80}$/.test(sku)) {
    throw new TypeError("sku must contain 1-80 safe identifier characters");
  }
}
...
export function createInventoryItem(sku: string, onHand = 0): InventoryItem {
  assertSku(sku);
  if (!Number.isSafeInteger(onHand) || onHand < 0) throw new RangeError("onHand must be a non-negative safe integer");
  return { sku, onHand, reserved: 0 };
}
...
export function applyInventoryOperation(item: InventoryItem, operation: InventoryOperation): InventoryItem {
  ...
  switch (operation.type) {
    case "receive":
      return { ...item, onHand: item.onHand + operation.quantity };
    case "reserve":
