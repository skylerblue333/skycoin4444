...
export class InMemoryQueue<T> {
  ...
  claim(now: string): QueueJob<T> | undefined {
    ...
    const eligible = [...this.jobs.values()]
      .filter((job) => job.attempts < job.maxAttempts && parseTime(job.availableAt, "availableAt") <= nowMs)
      .sort((a, b) => parseTime(a.availableAt, "availableAt") - parseTime(b.availableAt, "availableAt") || a.id.localeCompare(b.id));
