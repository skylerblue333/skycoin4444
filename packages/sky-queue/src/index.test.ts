...
describe("SkyQueue", () => {
  it("claims eligible jobs in deterministic order", () => {
    ...
    queue.enqueue({ id: "job:b", payload: { value: 2 }, availableAt: "2026-08-25T09:00:00.000Z" });
    queue.enqueue({ id: "job:a", payload: { value: 1 }, availableAt: "2026-08-25T09:00:00.000Z" });
  ...
  it("does not claim future or already-claimed jobs", () => {
    ...
    queue.enqueue({ id: "job:1", payload: "x", availableAt: "2026-08-25T10:00:00.000Z" });
  ...
  it("supports explicit retry and completion", () => {
    const queue = new InMemoryQueue<string>();
    queue.enqueue({ id: "job:1", payload: "x", availableAt: "2026-08-25T09:00:00.000Z", maxAttempts: 2 });
  ...
  it("rejects duplicate and malformed job inputs", () => {
    const queue = new InMemoryQueue<string>();
    queue.enqueue({ id: "job:1", payload: "x", availableAt: "2026-08-25T09:00:00.000Z" });
    expect(() => queue.enqueue({ id: "job:1", payload: "y", availableAt: "2026-08-25T09:00:00.000Z" })).toThrow("duplicate");
    expect(() => queue.enqueue({ id: "../bad", payload: "y", availableAt: "2026-08-25T09:00:00.000Z" })).toThrow("invalid job id");
  });
});
