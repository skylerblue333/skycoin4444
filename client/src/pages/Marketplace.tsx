  ...
  return (
    ...
    >
      <div className="relative aspect-square bg-black/20 overflow-hidden">
        ...
        {product.isHot && (
          <Badge className="absolute top-2 right-2 bg-orange-500/90 text-white text-[10px] px-1.5">
      ...
      <div className="p-3">
        <p className="text-sm font-medium line-clamp-2 mb-1.5 leading-snug">
          {product.title}
        </p>
        <div className="flex items-center gap-1.5 mb-2">
          {[1, 2, 3, 4, 5].map(i => (
            <Star
              key={i}
              className={`w-3 h-3 ${i <= Math.round(product.rating || 0) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/20"}`}
            />
          ))}
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            {discount > 0 && (
              <p className="text-[10px] text-muted-foreground line-through">
                ${(product.platformPrice / (1 - discount / 100)).toFixed(2)}
              </p>
            )}
          </div>
          <div className="text-right">
            ...
            <div className="flex items-center gap-0.5 text-[10px] text-muted-foreground">
              ...
              {product.deliveryDays || "14-21 days"}
            </div>
          </div>
        </div>
      </div>
...
function NativeModal({
  ...
  const placeOrder = trpc.marketplace.placeOrder.useMutation({
    onSuccess: data => {
      if (data.available && data.marketplaceUrl) {
        setOrdered(true);
        window.open(data.marketplaceUrl, "_blank", "noopener,noreferrer");
      } else {
        toast.error(data.message || "Marketplace ordering is unavailable.");
      }
    },
      ...
      >
        ...
        <div className="p-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              {[1, 2, 3, 4, 5].map(i => (
                <Star
                  key={i}
                  className={`w-3.5 h-3.5 ${i <= Math.round(product.rating || 0) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/20"}`}
                />
              ))}
              ...
              <div className="flex flex-wrap gap-2">
                {product.colors.map((c: string) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedColor === c ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-secondary/80"}`}
                  >
                    {c}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {product.sizes.map((s: string) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedSize === s ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-secondary/80"}`}
                  >
                    {s}
                  </button>
                ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-secondary rounded-xl p-1">
              <button
                ...
                className="w-7 h-7 rounded-lg hover:bg-background/50 font-bold"
              >
                −
              </button>
          ...
          <p className="text-[10px] text-muted-foreground text-center">
            Powered by Native · Platform fee supports SKYCOIN4444
          </p>
        </div>
      </div>
    </div>
...
function NativeTabContent() {
  ...
  const [sort, setSort] = useState<
    "featured" | "price_asc" | "price_desc" | "rating" | "sold"
  >("featured");
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.marketplace.getProducts.useQuery(
    { category: cat, search: search || undefined, sort, limit: 24, offset: 0 },
    { refetchOnWindowFocus: false }
  );
  const { data: hotProducts } = trpc.marketplace.getHot.useQuery(undefined, {
    retry: false,
  });
  ...
  return (
    <div className="space-y-4">
      {selected && (
        <NativeModal product={selected} onClose={() => setSelected(null)} />
      )}
      {/* Hot deals */}
          ...
          >
            {(hotProducts as any[]).slice(0, 5).map((p: any) => (
              <button
                ...
                className="flex-shrink-0 w-32 bg-black/20 rounded-xl overflow-hidden hover:ring-1 hover:ring-orange-500/40 transition-all text-left"
              >
                {p.imageUrl && (
                  <img
                    src={p.imageUrl}
                ...
                <div className="p-2">
                  <p className="text-xs line-clamp-1 font-medium">{p.title}</p>
                  <p className="text-sm font-bold text-primary">
                    ${p.platformPrice?.toFixed(2)}
                  </p>
                </div>
              </button>
            ))}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {list.map((p: any) => (
            <NativeCard key={p.id} product={p} onClick={() => setSelected(p)} />
          ))}
        </div>
      )}
      <p className="text-center text-xs text-muted-foreground py-2">
        Products sourced from Native when verified listings are available ·{" "}
        <a
          href="https://www.marketplace.com"
          target="_blank"
            ...
            <div className="flex items-center gap-3">
              ...
              {imageUrl && (
                <img
                  src={imageUrl}
                  alt=""
                  className="w-10 h-10 rounded object-cover border border-border/30"
                />
              )}
            </div>
          </div>
    ...
    <div className="p-5 rounded-xl border border-border/50 bg-card/80 backdrop-blur hover:border-primary/30 transition-all group relative">
      ...
      {item.rarity && (
        <div className="absolute top-3 left-3">
          <Badge
            className={`text-[10px] ${item.rarity === "Legendary" ? "bg-yellow-500/10 text-yellow-400 border-yellow-500/30" : "bg-blue-500/10 text-blue-400 border-blue-500/30"}`}
          >
            {item.rarity}
          </Badge>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
          ...
          {item.isAuction && (
            <span className="flex items-center gap-0.5">
              <Gavel className="w-3 h-3" /> {item.bids} bids
            </span>
          )}
        </div>
    ...
    onSuccess: (data: any) => {
      if (data.url) {
        if (data.available && data.url) {
          ...
          toast.error(data.message || "Secure checkout is unavailable.");
        }
      }
    },
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-purple-500/30 bg-purple-500/5 mb-4">
                ...
                <span className="text-xs font-mono text-[oklch(0.7_0.2_280)]">
              ...
              <p className="text-muted-foreground">
                Buy, sell, and auction digital assets with escrow protection in
              ...
              {isLoading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[1, 2, 3, 4, 5, 6].map(i => (
                    <div
                      key={i}
                      className="p-4 rounded-xl border border-border/50 animate-pulse"
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {listings.map((listing: any) => (
                    <div
                      key={listing.id}
                      className="p-5 rounded-xl border border-border/50 bg-card/80 hover:border-primary/30 transition-all group"
                    >
                      <div className="h-36 rounded-lg bg-gradient-to-br from-primary/10 to-[oklch(0.15_0.03_280)] flex items-center justify-center mb-4 overflow-hidden">
                        {listing.imageUrl ? (
                          <img
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-muted-foreground text-sm mb-6 max-w-md mx-auto">
                    Be the first to list a digital asset, NFT, or service in the
                    SKYCOIN4444 marketplace!
                  </p>
                  {isAuthenticated ? (
                    <CreateListingDialog />
                  ) : (
              ...
              <div className="p-4 rounded-xl border border-orange-500/20 bg-orange-500/5 mb-6 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Bid on exclusive items. Highest bidder wins when the timer
                    expires. All bids are secured via escrow.
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="mt-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-5 rounded-xl border border-border/50 bg-card/80 text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  Funds held securely until buyer confirms delivery. Dispute
              ...
              <div className="p-5 rounded-xl border border-border/50 bg-card/80 text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  All sellers undergo verification. Reputation system ensures
                  quality.
                </p>
              </div>
              <div className="p-5 rounded-xl border border-border/50 bg-card/80 text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  Automated settlement via audited smart contracts. No
                  intermediaries.
                </p>
              </div>
            </div>
