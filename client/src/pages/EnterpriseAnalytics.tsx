    ...
    <Card className="bg-zinc-900/60 border-zinc-800">
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {change !== undefined && (
            <div
              className={`flex items-center gap-1 text-xs font-medium ${change >= 0 ? "text-emerald-400" : "text-red-400"}`}
            >
              {change >= 0 ? (
...
export default function ScalableAnalytics() {
  ...
  const securityQuery = trpc.enterprise.security.myRiskScore.useQuery(
    undefined,
    {
      enabled: !!user,
  ...
  const economyQuery = trpc.enterprise.economy.marketStates.useQuery(
    undefined,
    {
      enabled: !!user,
      refetchInterval: 30_000,
  ...
  const security = securityQuery.data as unknown as
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-500">
                Platform Intelligence — Real-time Ecosystem Metrics
              </p>
            </div>
          </div>
          <Button
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <KPICard
            ...
            value={(analytics?.dau as number)?.toLocaleString() ?? "8,412"}
            change={5.2}
            icon={Users}
            color="text-blue-400"
          />
          <KPICard
            ...
            value={((analytics?.tokenVelocity as number) ?? 2.4).toFixed(2)}
            suffix="x"
            change={1.8}
            icon={Zap}
          ...
          <KPICard
            ...
            value={((analytics?.retention7d as number) ?? 67.3).toFixed(1)}
            suffix="%"
            change={-2.1}
            icon={Target}
          ...
          <KPICard
            label="AI Actions/hr"
            value={(analytics?.aiActionsPerHour as number) ?? 4280}
            change={12.4}
            icon={Brain}
                  ...
                  {[
                    {
                      ...
                      value: `${((economy?.circulatingSupply as number) ?? 45_000_000).toLocaleString()} SKY`,
                    },
                    {
                      label: "Staking Rate",
                      value: `${((economy?.stakingRate as number) ?? 38.4).toFixed(1)}%`,
                    },
                    {
                      label: "Burn Rate (30d)",
                      value: `${((economy?.burnRate30d as number) ?? 45_000).toLocaleString()} SKY`,
                    },
                    {
                      label: "Inflation Rate",
                      value: `${((economy?.inflationRate as number) ?? 2.1).toFixed(2)}%/yr`,
                    },
                  ].map(s => (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge
                            className={`text-[10px] ${agent.success >= 99 ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : "bg-amber-500/10 text-amber-400 border-amber-500/30"}`}
                          >
