...
export function toSavedDatingProfile(
  ...
  return {
    ...persistable,
    photoCount: photos.length || previousPhotoCount,
    savedAt: new Date().toISOString(),
    storage: "browser-session",
  };
}

export function parseSavedDatingProfile(
  value: string | null
): SavedProfile | null {
  if (!value) return null;
  try {
    const parsed = JSON.parse(value) as Partial<SavedProfile>;
    if (
      ...
      !parsed.interests.every(interest => typeof interest === "string") ||
      !["unverified", "email", "phone", "id"].includes(
...
export default function DatingProfileSetup() {
  ...
  useEffect(() => {
    ...
    const {
      photoCount,
      savedAt: restoredSavedAt,
      storage: _storage,
      ...savedProfile
    } = restored;
    setFormData({ ...savedProfile, photos: [] });
  ...
  const handlePhotoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    const newPhotos = Array.from(event.target.files ?? []).filter(file =>
      ...
      <div className="mx-auto max-w-2xl">
        ...
        {step === 2 && (
          <Card className="space-y-4 p-6">
            ...
            <p className="text-sm text-gray-600">
              Choose up to six local images. Images are previewed in-memory and
              are not uploaded by this engineering-beta flow.
            </p>
            {restoredPhotoCount > 0 && (
              <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
                The saved draft recorded {restoredPhotoCount} local photo
                selection(s). Browser file handles cannot be restored; choose
                them again before a new preview.
              </p>
            )}
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              ...
              {formData.photos.length < 6 && (
                <label className="flex aspect-square cursor-pointer items-center justify-center rounded-lg border-2 border-dashed border-pink-300 bg-pink-50 hover:bg-pink-100">
                  <div className="text-center">
                    <Upload className="mx-auto mb-2 h-6 w-6 text-pink-500" />
        ...
        {step === 5 && (
          <Card className="space-y-4 p-6">
            ...
            <p className="text-sm text-gray-600">
              These controls record a preference only. They do not perform
              email, phone, or government-ID verification.
            </p>
            <div className="grid gap-3">
              {(
                ["unverified", "email", "phone", "id"] as VerificationStatus[]
              ).map(status => (
                <Button
                  type="button"
                  key={status}
                ...
                >
                  {status === "unverified"
                    ? "Skip verification"
                    : `Prefer ${status} verification`}
                </Button>
              ))}
            </div>
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
              <ShieldCheck className="mb-2 h-5 w-5" />A production verification
              provider and server-side profile store remain separate
              integrations.
            </div>
...
function PhotoPreview({
  ...
  return (
    <div className="relative aspect-square overflow-hidden rounded-lg bg-gray-100">
      <img
        src={url}
        alt={photo.name || "Profile preview"}
