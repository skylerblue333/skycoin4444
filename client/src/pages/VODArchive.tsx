...
export default function VODArchive() {
  ...
  const { data: vodsRaw, isLoading } = trpc.stream.vods.useQuery(undefined, {
    staleTime: 60_000,
  });
  ...
  const vods = useMemo(() => {
    ...
    if (list.length === 0) {
      list = [
        {
          id: 1,
          title: "DeFi Yield Strategies 2025 — Full Masterclass",
          streamerName: "CryptoSkyler",
          viewerCount: 48200,
          duration: 8073,
          category: "Education",
        ...
        {
          id: 2,
          title: "SKY444 Token Deep Dive — Live Analysis",
          streamerName: "SkyTrader",
          viewerCount: 31500,
          duration: 5400,
          category: "Crypto",
          startedAt: new Date("2025-06-08"),
          endedAt: new Date(),
          status: "ended",
        },
        {
        ...
        {
          id: 6,
          title: "Crypto Portfolio Management 101",
          streamerName: "PortfolioSky",
          viewerCount: 19200,
          duration: 4500,
          category: "Education",
          startedAt: new Date("2025-05-30"),
          endedAt: new Date(),
          status: "ended",
        },
      ];
    ...
    if (category !== "All") {
      list = list.filter(
        (v: any) => (v.category || "").toLowerCase() === category.toLowerCase()
      );
    }
  ...
  return (
    <div className="min-h-screen">
      <PageHeader
        title="VOD Archive"
        subtitle="Browse past streams, replays, and recorded sessions"
      />
      <div className="container py-6 max-w-7xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            {
              label: "Total VODs",
              value: vods.length.toLocaleString(),
              icon: Tv,
            },
            {
              label: "Total Views",
              value: formatViews(
                vods.reduce((s: number, v: any) => s + (v.viewerCount || 0), 0)
              ),
              icon: Eye,
            },
            {
              label: "Hours of Content",
              value: `${Math.round(vods.reduce((s: number, v: any) => s + (v.duration || 0), 0) / 3600)}h`,
              icon: Clock,
            },
            {
              label: "Creators",
              value: new Set(
                vods.map((v: any) => v.streamerName || v.creator)
              ).size.toString(),
              icon: Grid,
            },
        ...
        {isLoading ? (
          ...
          >
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="animate-pulse bg-white/5 rounded-xl h-48"
              />
            ))}
          </div>
        ) : paginated.length === 0 ? (
                ...
                <CardContent className="p-0">
                  ...
                  <div className="p-3">
                    <h3 className="font-medium text-sm line-clamp-2 mb-1">
                      {vod.title || `Stream #${vod.id}`}
                    </h3>
                    <p className="text-xs text-muted-foreground mb-2">
                      {vod.streamerName || vod.creator || "Creator"}
                    </p>
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Eye className="w-3 h-3" />
                        {formatViews(vod.viewerCount || 0)}
                      </span>
                      {vod.category && (
          ...
          <DialogContent className="max-w-3xl bg-zinc-950 border-white/10">
            <DialogHeader>
              <DialogTitle className="text-lg">
                {selected.title || `Stream #${selected.id}`}
              </DialogTitle>
            </DialogHeader>
            <div className="bg-gradient-to-br from-purple-900/30 to-cyan-900/30 rounded-xl h-64 flex items-center justify-center mb-4">
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Connect a CDN/HLS source to enable playback
                </p>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium">
                  {selected.streamerName || "Creator"}
                </p>
                <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
                  <span className="flex items-center gap-1">
                    <Eye className="w-3 h-3" />
                    {formatViews(selected.viewerCount || 0)} views
                  </span>
                  {selected.duration && (
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3" />
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => {
                    navigator.clipboard.writeText(
                      window.location.href + `?vod=${selected.id}`
                    );
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() =>
                    toast.info("Download requires VOD CDN integration")
                  }
                >
