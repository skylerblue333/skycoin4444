        ...
        <div className="container max-w-6xl mx-auto px-4 relative z-10">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground metallic-shimmer text-sm">
                Real-time global charity impact — where your donations are
                changing lives
              </p>
            </div>
          </div>
      ...
      <div className="container max-w-6xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div
              className="rounded-xl border border-white/10 bg-white/3 p-6 relative overflow-hidden"
              style={{ minHeight: 360 }}
            >
              <div className="absolute inset-0 cyber-grid opacity-10" />
              <h2 className="text-lg font-bold text-white mb-4 relative z-10">
                    ...
                    >
                      <div
                        ...
                        style={{
                          width: size,
                          height: size,
                          background: `rgba(${r.color.includes("green") ? "34,197,94" : r.color.includes("blue") ? "59,130,246" : r.color.includes("rose") ? "244,63,94" : r.color.includes("amber") ? "245,158,11" : r.color.includes("purple") ? "168,85,247" : "6,182,212"},0.4)`,
                        }}
                      />
                    </button>
                  );
          ...
          <div className="space-y-4">
            {selected ? (
              <div
                className={`rounded-xl border ${selected.border} ${selected.bg} p-5`}
              >
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="h-2 rounded-full bg-white/10">
                      <div
                        className={`h-2 rounded-full ${selected.bg.replace("bg-", "bg-").replace("/20", "/60")}`}
                        style={{
                          width: `${(selected.raised / selected.goal) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-lg bg-white/5 p-2 text-center">
                      <div className={`text-lg font-black ${selected.color}`}>
                        {selected.campaigns}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        Campaigns
                    ...
                    <div className="rounded-lg bg-white/5 p-2 text-center">
                      <div className={`text-lg font-black ${selected.color}`}>
                        {selected.beneficiaries.toLocaleString()}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        Helped
                      </div>
                  ...
                  <a
                    href="/charity"
                    className={`block text-center text-xs font-bold py-2 rounded-lg ${selected.bg} ${selected.border} border ${selected.color} hover:opacity-80 transition-opacity`}
                  >
                    View Campaigns →
                  </a>
                </div>
              ...
              <div className="rounded-xl border border-white/10 bg-white/3 p-5 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Click a region on the map to see detailed impact stats
                </p>
              </div>
            )}
        ...
        {/* Region cards grid */}
        <div className="mt-8">
          <h2 className="text-xl font-bold text-white mb-4 neon-drip">
            All Regions
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {REGIONS.map(r => (
              <div
                key={r.id}
                className={`rounded-xl border ${r.border} ${r.bg} p-4 cursor-pointer hover:scale-[1.02] transition-transform`}
              ...
              >
                ...
                <div className="h-1.5 rounded-full bg-white/10 mb-2">
                  <div
                    ...
                    style={{
                      width: `${(r.raised / r.goal) * 100}%`,
                      color: r.color.replace("text-", ""),
                    }}
                  />
                </div>
                <div className="flex justify-between text-xs">
                  <span className={`font-bold ${r.color}`}>
                    ${(r.raised / 1000).toFixed(0)}K raised
                  </span>
                  <span className="text-muted-foreground">
                    {r.beneficiaries.toLocaleString()} helped
                  </span>
                </div>
              </div>
            ))}
