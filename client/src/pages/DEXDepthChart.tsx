...
function generateDepthData(midPrice: number, spread = 0.02) {
  ...
  for (let i = 50; i >= 1; i--) {
    ...
    bids.push({
      ...
      cumBid: parseFloat((cumBid / 1000).toFixed(2)),
      bid: parseFloat((size / 1000).toFixed(2)),
    });
  }
  ...
  for (let i = 1; i <= 50; i++) {
    ...
    asks.push({
      price: parseFloat(price.toFixed(4)),
      cumAsk: parseFloat((cumAsk / 1000).toFixed(2)),
      ask: parseFloat((size / 1000).toFixed(2)),
    });
  }
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs">
                Depth charts · Order books · Pool analytics
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              variant="outline"
              className="border-white/10 text-slate-400 h-9"
            >
          ...
          {[
            {
              ...
              value:
                pair.price > 1000
                  ...
                  : `$${pair.price}`,
              sub: `${pair.change >= 0 ? "+" : ""}${pair.change}%`,
              color: pair.change >= 0 ? "text-green-400" : "text-red-400",
            },
            {
              label: "24h Volume",
              value: `$${(pair.volume / 1000000).toFixed(2)}M`,
              sub: "24h",
              color: "text-cyan-400",
            },
            {
              label: "TVL",
              value: `$${(pair.tvl / 1000000).toFixed(1)}M`,
            ...
            },
            {
              label: "Spread",
              value: `${spreadPct}%`,
              sub: `$${spread.toFixed(4)}`,
              color: "text-yellow-400",
            },
          ].map(stat => (
            <div
              key={stat.label}
              className="bg-[#0e0a1a] border border-white/5 rounded-xl p-3"
            >
            ...
            <div className="bg-[#0e0a1a] border border-white/5 rounded-2xl p-4">
              ...
              <div className="space-y-0.5">
                ...
                {topAsks.reverse().map((ask, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-3 text-xs px-1 py-0.5 hover:bg-red-500/5 rounded relative"
                  >
                    <div
                      className="absolute inset-0 right-0 bg-red-500/5 rounded"
                      style={{
                        width: `${Math.min((ask.cumAsk / (topAsks[topAsks.length - 1]?.cumAsk || 1)) * 100, 100)}%`,
                        right: 0,
                        left: "auto",
                      }}
                    />
                    <span className="text-red-400 font-mono relative z-10">
                      ${ask.price.toFixed(pair.price > 100 ? 2 : 4)}
                    </span>
                ...
                <div className="py-2 text-center">
                  ...
                  <Badge
                    className={`ml-2 text-[9px] ${pair.change >= 0 ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"} border-0`}
                  >
                ...
                {topBids.map((bid, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-3 text-xs px-1 py-0.5 hover:bg-green-500/5 rounded relative"
                  >
                    <div
                      className="absolute inset-0 bg-green-500/5 rounded"
                      style={{
                        width: `${Math.min((bid.cumBid / (topBids[topBids.length - 1]?.cumBid || 1)) * 100, 100)}%`,
                      }}
                    />
                    <span className="text-green-400 font-mono relative z-10">
                      ${bid.price.toFixed(pair.price > 100 ? 2 : 4)}
                    </span>
                    <span className="text-right text-slate-400 relative z-10">
                      {bid.bid}
                    </span>
        ...
        {tab === "pools" && (
          <div className="space-y-3">
            {PAIRS.map(p => (
              <div
                key={p.id}
                className="bg-[#0e0a1a] border border-white/5 rounded-xl p-4 flex items-center justify-between hover:border-purple-500/20 transition-all cursor-pointer"
                onClick={() => {
              ...
              >
                ...
                <div className="text-right">
                  ...
                  >
                    {p.change >= 0 ? "+" : ""}
                    {p.change}% · Vol ${(p.volume / 1000000).toFixed(1)}M
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    className="bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs h-7"
                  >
                    Add Liquidity
                  </Button>
                  <Button
                    size="sm"
                    className="bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 text-xs h-7"
                  >
