...
function ReputationBadge({ score }: { score: number }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${tier.bg} ${tier.color}`}
    >
...
export default function Profile() {
  ...
  const { data: profileData, isLoading } = username
    ? trpc.user.profileByUsername.useQuery(
        { username },
        { enabled: !!username }
      )
  ...
  const { data: achievements } = trpc.gamefi.achievements.useQuery(undefined, {
  ...
  const uploadAvatar = trpc.user.uploadAvatar.useMutation({
    onSuccess: (data, vars) => {
      toast.success(
        vars.type === "avatar" ? "Avatar updated!" : "Banner updated!"
      );
    },
  ...
  const userPosts =
    myPosts?.filter((p: any) => p.authorId === profile?.id) || [];
      ...
      <div className="relative h-48 md:h-64 overflow-hidden">
        {banner ? (
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, oklch(0.25 0.15 305), oklch(0.20 0.12 340), oklch(0.18 0.10 200))",
            }}
          />
        )}
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="flex items-center gap-2 pb-2">
              {isOwnProfile ? (
                <Link href="/profile-edit">
                  <Button
                    size="sm"
                    variant="outline"
                    className="border-white/10 text-white/70 hover:text-white hover:border-white/20 bg-transparent gap-1.5"
                  >
                ...
                <Button
                  size="sm"
                  onClick={() => followUser.mutate({ userId: profile.id })}
              ...
              <Button
                size="sm"
                variant="outline"
                className="border-white/10 text-white/40 hover:text-white bg-transparent"
                onClick={() => {
          ...
          <div className="mt-3">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-white/40 mb-4">
              ...
              {profile.twitter && (
                ...
                >
                  <TwitterIcon className="w-3 h-3" />@{profile.twitter}
                </a>
              )}
              {profile.instagram && (
                <a
                ...
                >
                  <InstagramIcon className="w-3 h-3" />@{profile.instagram}
                </a>
              )}
              {profile.youtube && (
                <a
            ...
            <div className="flex items-center gap-6 text-sm">
              {[
                {
                  label: "Posts",
                  value: profile.postCount || userPosts.length || 0,
                },
              ...
              <div className="bg-[#0e0a1a]/90 border border-white/5 rounded-2xl p-12 text-center">
                ...
                {isOwnProfile && (
                  <Link href="/social">
                    <Button
                      size="sm"
                      className="mt-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white border-0"
                    >
                      Create First Post
                    </Button>
                  </Link>
              ...
              <div className="grid grid-cols-3 gap-2">
                {userPosts
                  ...
                  .map((post: any) => (
                    <div
                      key={post.id}
                      className="aspect-square rounded-xl overflow-hidden bg-black/30 relative group cursor-pointer"
                    >
                      <img
                        src={post.mediaUrl}
                        alt=""
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div
                    key={s.label}
                    className="bg-[#0e0a1a]/90 border border-white/5 rounded-2xl p-4"
                  >
              ...
              <div className="bg-[#0e0a1a]/90 border border-white/5 rounded-2xl p-4">
                ...
                <ResponsiveContainer width="100%" height={140}>
                  <BarChart
                    data={CREATOR_ANALYTICS.filter((_, i) => i % 3 === 0)}
                    margin={{ top: 5, right: 5, left: 0, bottom: 0 }}
                  >
                    <XAxis
                      dataKey="date"
