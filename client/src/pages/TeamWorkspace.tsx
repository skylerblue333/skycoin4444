...
export default function TeamWorkspace() {
  ...
  const { data: communities } = trpc.community.list.useQuery({});
  ...
  return (
    <div className="min-h-screen bg-[#050510] text-white">
      {/* ═══ CINEMATIC HERO ═══ */}
      <div
        className="relative overflow-hidden border-b border-blue-500/20"
        style={{
          background:
            "linear-gradient(135deg, oklch(0.08 0.04 240), oklch(0.06 0.03 270), oklch(0.08 0.04 200))",
        }}
      >
        <div className="absolute inset-0 pointer-events-none">
          <div
            className="glow-orb w-64 h-64 -top-20 -left-20 opacity-20"
            style={{ background: "oklch(0.65 0.25 240)" }}
          />
          <div
            className="glow-orb w-48 h-48 -bottom-16 right-20 opacity-15"
            style={{ background: "oklch(0.65 0.25 200)" }}
          />
        </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-3">
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, oklch(0.65 0.25 240), oklch(0.65 0.25 200))",
                    boxShadow: "0 0 24px oklch(0.65 0.25 240 / 0.4)",
                  }}
                >
                  <Users className="w-6 h-6 text-white" />
                </div>
                <div>
                  <h1 className="text-3xl font-black text-rainbow">
                    Team Workspace
                  </h1>
                  <p className="text-white/50 text-sm">
                    团队协作空间 · Collaborate, Build, Ship Together
                  </p>
                </div>
              </div>
              <p className="text-white/60 max-w-xl">
                Your unified workspace for team communication, project tracking,
                file sharing, and real-time collaboration. Built for builders.
              </p>
            </div>
            <div className="hidden md:flex gap-3">
              <div className="text-center px-4 py-2 rounded-xl border border-blue-500/20 bg-blue-500/10">
                <div className="text-2xl font-black text-blue-300 stat-number">
                  24
                </div>
              ...
              <div className="text-center px-4 py-2 rounded-xl border border-green-500/20 bg-green-500/10">
                <div className="text-2xl font-black text-green-300 stat-number">
                  8
                </div>
              ...
              <div className="text-center px-4 py-2 rounded-xl border border-purple-500/20 bg-purple-500/10">
                <div className="text-2xl font-black text-purple-300 stat-number">
                  5
                </div>
        ...
        <Tabs defaultValue="channels">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className="bg-white/10 text-white/50 text-xs">
                          {
                            DEMO_CHANNELS.find(c => c.id === activeChannel)
                              ?.members
                          }{" "}
                          members
                        </Badge>
                    ...
                    {[
                      {
                        user: "Alex Chen",
                        avatar: "AC",
                        msg: "Just pushed the new auth flow to staging 🚀",
                        time: "10:24 AM",
                      ...
                      },
                      {
                        user: "Sam Park",
                        avatar: "SP",
                        msg: "Looks great! Testing now. The rate limiting is also done.",
                        time: "10:26 AM",
                      ...
                      },
                      {
                        user: "Jordan Lee",
                        avatar: "JL",
                        msg: "PM update: Q2 analytics report is complete. Check the dashboard.",
                        time: "10:31 AM",
                        color: "oklch(0.65 0.25 70)",
                      },
                      {
                        user: "Riley Zhang",
                        avatar: "RZ",
                        msg: "New design mockups for the onboarding flow are in Figma 🎨",
                        time: "10:45 AM",
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {filteredTasks.map(task => (
                  <Card
                    key={task.id}
                    className="bg-white/5 border-white/10 hover:border-blue-500/30 transition-all"
                  >
                    <CardContent className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <CheckSquare
                          className={`w-4 h-4 ${task.status === "done" ? "text-green-400" : "text-white/30"}`}
                        />
                        <div>
                          <p
                            className={`font-medium text-sm ${task.status === "done" ? "line-through text-white/40" : "text-white"}`}
                          >
                            {task.title}
                          </p>
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-xs font-bold ${PRIORITY_COLORS[task.priority]}`}
                        >
          ...
          <TabsContent value="members">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                >
                  <CardContent className="p-4 flex items-center gap-3">
                    <div className="relative">
                      ...
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#050510] ${MEMBER_STATUS_COLORS[member.status]}`}
                      />
                    </div>
              ...
              <Card
                className="bg-white/5 border-white/10 border-dashed hover:border-blue-500/30 transition-all cursor-pointer"
                onClick={() =>
                  toast.info("Invite team members via email or link")
                }
              >
          ...
          <TabsContent value="files">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                <Card
                  key={i}
                  className="bg-white/5 border-white/10 hover:border-blue-500/30 transition-all cursor-pointer"
                >
                  <CardContent className="p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <Badge className="bg-white/10 text-white/50 text-[10px] px-1.5">
                          {file.type}
                        </Badge>
                        <span className="text-xs text-white/30">
                          {file.size}
              ...
              {[
                ...
                <Card key={i} className="bg-white/5 border-white/10">
                  <CardContent className="p-4">
                    ...
                    <div
                      className={`text-2xl font-black stat-number ${stat.color}`}
                    >
                      {stat.value}
                    </div>
                    ...
                    <div key={m.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-xs text-white/40">
                            {Math.floor(Math.random() * 50 + 20)} actions
                          </span>
                        </div>
