...
export default function UniversalSearch() {
  ...
  const backendResults = searchData
    ? [
        ...(searchData.users ?? []).map((u: any) => ({
          ...
          sub: `@${u.username ?? u.openId} · ${u.role}`,
          href: "/profile",
          badge: u.verified ? "VERIFIED" : null,
        })),
        ...(searchData.posts ?? []).map((p: any) => ({
          type: "posts",
          title: p.title ?? p.content?.slice(0, 60),
          sub: `${p.likes ?? 0} likes · ${p.commentsCount ?? 0} comments`,
          href: "/social",
          badge: null,
        })),
        ...(searchData.communities ?? []).map((c: any) => ({
          type: "communities",
          title: c.name,
  ...
  const displayResults = debouncedQuery.length >= 2 ? backendResults : [];
  const filteredResults =
    cat === "all"
      ? displayResults
      : displayResults.filter(
          (r: any) => r.type === cat || r.type === cat.replace(/s$/, "")
        );
            ...
            <div className="flex flex-wrap gap-2">
              {TRENDING.map(t => (
                <button
                  key={t}
                  onClick={() => setQuery(t)}
                  className="bg-slate-900/60 border border-slate-800 hover:border-emerald-500/50 text-slate-300 text-xs px-3 py-1.5 rounded-lg transition-all hover:text-emerald-400"
                >
                  #{t}
                </button>
              ))}
            ...
            <div className="space-y-1">
              {recentSearches.map(s => (
                <button
                  key={s}
                  onClick={() => setQuery(s)}
                  className="flex items-center gap-3 w-full text-left px-3 py-2 rounded-lg hover:bg-slate-900/60 text-slate-400 hover:text-slate-200 text-sm transition-all"
                >
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-semibold text-slate-300">
                ...
                {query && (
                  <span className="text-slate-500 ml-2">for "{query}"</span>
                )}
              </h2>
            </div>
            <div className="space-y-2">
              {filteredResults.map((r: any, i: number) => {
                const Icon =
                  CATEGORIES.find(c => c.id === r.type)?.icon ?? Search;
                ...
                return (
                  <Link key={i} href={r.href ?? "/"}>
                    <div className="flex items-center gap-4 p-4 bg-slate-900/40 rounded-xl border border-slate-800/60 hover:border-slate-700 cursor-pointer transition-all group">
                      <div
                        className={`h-9 w-9 rounded-lg bg-slate-800 flex items-center justify-center shrink-0`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-white text-sm truncate">
                            {r.title ?? r.name ?? r.username}
                          </span>
                          {r.badge && (
                            <Badge
                              className={`text-xs border-0 px-1.5 py-0 ${r.badge === "LIVE" ? "bg-rose-500/15 text-rose-400" : r.badge === "VERIFIED" ? "bg-blue-500/15 text-blue-400" : r.badge === "HOT" ? "bg-amber-500/15 text-amber-400" : "bg-slate-800 text-slate-400"}`}
                            >
                              {r.badge}
                            </Badge>
                          )}
