...
function Block({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-white/[0.02] p-4">
      <h4 className="text-sm font-semibold mb-2" style={{ color: GOLD }}>
        {title}
      </h4>
      {children}
    </div>
...
export function StartupSection() {
  ...
  const detail = trpc.hopeIntelligence.startup.get.useQuery(
    { id: activeId! },
  ...
  const roadmap = (bp?.mvpRoadmap ?? []) as Array<{
    phase: string;
    items: string[];
  }>;
  ...
  return (
    <div className="grid lg:grid-cols-[1fr_1.4fr] gap-6">
        ...
        <Card className="border-white/10 bg-white/[0.02]">
          ...
          <CardContent className="space-y-3">
            ...
            <Button
              ...
              disabled={generate.isPending || idea.trim().length < 8}
              onClick={() => generate.mutate({ idea: idea.trim() })}
            >
              {generate.isPending ? (
                <>
            ...
            {list.isLoading ? (
              ...
              list.data.map(b => (
                <button
                  ...
                  className={`w-full text-left rounded-lg border px-3 py-2 transition-colors ${activeId === b.id ? "border-amber-500/50 bg-amber-500/5" : "border-white/10 hover:bg-white/[0.04]"}`}
                >
      ...
      <div>
        {activeId == null ? (
          <EmptyState
            title="Select or generate a blueprint"
            hint="HOPE AI turns your idea into a plan, branding, marketing, an MVP roadmap, and a team plan."
          />
        ) : detail.isLoading ? (
          <SectionLoading label="Loading blueprint…" />
        ) : bp ? (
                ...
                <div className="space-y-2">
                  {roadmap.map((r, i) => (
                    <div key={i}>
                      <Badge
                        variant="outline"
                        className="border-white/20 text-white/70 text-xs mb-1"
                      >
                        {r.phase}
                      </Badge>
