      ...
      {content == null ? (
        <div className="space-y-4">
          <p className="text-sm text-white/60">
            Purchase to unlock the full content. Payment settles instantly in
            SKY444.
          </p>
          <Button
            className="w-full"
            ...
            onClick={() => purchase.mutate({ listingId })}
          >
            {purchase.isPending ? (
        ...
        <div className="space-y-4">
          <pre className="whitespace-pre-wrap rounded-lg border border-white/10 bg-black/40 p-3 text-sm text-white/80 max-h-72 overflow-auto">
            {content}
          </pre>
          <div className="flex items-center gap-1">
            ...
            {[1, 2, 3, 4, 5].map(s => (
              <button
                key={s}
                onClick={() => rate.mutate({ listingId, stars: s })}
...
function CreateListing() {
  ...
  const [form, setForm] = useState({
    kind: "prompt" as (typeof KINDS)[number],
    title: "",
    description: "",
    content: "",
    priceCoin: "0",
  ...
  const submit = () => {
    ...
    create.mutate({
      ...
      priceCents: isNaN(priceCents) ? 0 : Math.max(0, priceCents),
    });
  };
    ...
    <DialogContent className="bg-[#0a0a14] border-white/10 text-white">
      ...
      <div className="space-y-3">
        <Select
          value={form.kind}
          onValueChange={v =>
            setForm(f => ({ ...f, kind: v as typeof f.kind }))
          }
        >
        ...
        />
        <Textarea
          placeholder="The paid content (prompt text, workflow JSON, agent config…)"
          rows={5}
          value={form.content}
        ...
        <Input
          ...
          min="0"
          step="0.01"
          placeholder="Price in SKY444 (0 = free)"
          value={form.priceCoin}
          onChange={e => setForm(f => ({ ...f, priceCoin: e.target.value }))}
          className="bg-white/5 border-white/10"
        />
        <Button
          className="w-full"
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-white/70">
          ...
          <span className="font-semibold text-white">
            {balance.data
              ? `${balance.data.balance.toLocaleString()} SKY444`
              : "…"}
          </span>
        </div>
        <Dialog>
          <DialogTrigger asChild>
            <Button size="sm" style={{ backgroundColor: GOLD, color: "#000" }}>
      ...
      {list.isLoading ? (
        ...
        <EmptyState
          ...
          hint="Be the first to publish a prompt, agent, workflow, or template."
        />
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.data.map(l => (
            <Card
              key={l.id}
              className="border-white/10 bg-white/[0.02] flex flex-col"
            >
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <Badge
                    variant="outline"
                    className="border-white/20 text-white/50 text-[10px] capitalize"
                  >
                    {l.kind}
                  </Badge>
                  <span className="flex items-center gap-1 text-xs text-white/50">
                    <Star className="h-3 w-3" style={{ color: GOLD }} />
                    {avg(l.ratingSum, l.ratingCount)} ({l.ratingCount})
                  </span>
                </div>
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                      size="sm"
                      variant="outline"
                      className="mt-3 border-white/20 text-white/80"
                    >
