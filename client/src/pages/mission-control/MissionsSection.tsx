...
const CAPABILITIES = [
  "Authenticated mission persistence",
  "AI-generated step plans",
  "Step completion and progress tracking",
  "Mission categories and status",
] as const;
  ...
  if (!isAuthenticated || !user) {
    return (
      <Card className="border-white/10 bg-white/[0.02]">
        <CardHeader><CardTitle className="text-white">Missions</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-white/50">Sign in to view account-scoped missions.</p>
    ...
    <div className="space-y-6">
      ...
      <Card className="border-amber-400/30 bg-amber-400/[0.06]">
        <CardContent className="p-6">
          <h3 className="font-semibold text-amber-100">Mission services unavailable</h3>
          <p className="mt-2 text-sm leading-6 text-white/60">
            No verified mission backend is currently exposed. This page does not create missions, generate AI plans, report progress, or claim that steps have been persisted or completed.
          </p>
        </CardContent>
      </Card>
      <Card className="border-white/10 bg-white/[0.02]">
