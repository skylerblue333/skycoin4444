      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-2 text-xs flex-wrap">
            ...
            <div
              className="flex items-center gap-1 px-2 py-1 rounded-lg bg-secondary/60"
...
function IncomeFlowItem({ event }: { event: IncomeEvent }) {
  return (
    <div className="flex items-center gap-3 py-2 border-b border-border/30 last:border-0">
      <div
        className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${event.direction === "in" ? "bg-green-500/20" : "bg-red-500/20"}`}
      >
        {event.direction === "in" ? (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-sm font-bold ${event.direction === "in" ? "text-green-400" : "text-red-400"}`}
          >
            {event.direction === "in" ? "+" : "-"}$
...
export default function ActionObjects() {
  const [activeTab, setActiveTab] = useState<"catalog" | "history" | "flow">(
    "catalog"
  );
  ...
  const balance =
    walletQuery.data?.balances?.find(item => item.token === "SKY444")
      ?.balance ?? 0;
  ...
  useEffect(() => {
    const mockFlow: IncomeEvent[] = [
      ...
      },
      {
        ...
        source: "Dating Premium",
      },
      {
        id: "6",
        label: "Creator tip sent",
        amount: 2,
        direction: "out",
  ...
  // Income flow totals
  const totalIn = incomeFlow
    .filter(e => e.direction === "in")
    .reduce((s, e) => s + e.amount, 0);
  const totalOut = incomeFlow
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Every action: cost → impact → result
            </p>
          </div>
        </div>
      ...
      {activeTab === "catalog" && (
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground px-1">
            Tap any action to execute. Every action shows cost → impact → result
            before you commit.
          </p>
          ...
          {actionHistory.length === 0 ? (
            <div className="card p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                No actions yet. Execute an action from the catalog.
              </p>
            </div>
          ) : (
