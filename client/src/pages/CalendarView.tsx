...
import { buildMonthCells, shiftSupportedMonth } from "@/lib/betaPresentationLab";
...
export default function CalendarView(){
  ...
  const previousMonth=shiftSupportedMonth(year,month,-1);
  const nextMonth=shiftSupportedMonth(year,month,1);
  const move=(delta:number)=>{
    const target=shiftSupportedMonth(year,month,delta);
    if(!target) return;
    setYear(target.year);
    setMonth(target.monthIndex);
    setSelected(null);
  };
  return <main className="min-h-screen bg-background p-4 md:p-8"><div className="mx-auto max-w-3xl space-y-6">
    ...
    <Card><CardHeader><div className="flex items-center justify-between gap-3"><Button size="icon" variant="outline" disabled={!previousMonth} onClick={()=>move(-1)} aria-label="Previous month"><ChevronLeft className="h-4 w-4"/></Button><div className="text-center"><CalendarDays className="mx-auto h-5 w-5 text-primary"/><CardTitle className="mt-2">{label}</CardTitle><CardDescription>{selected ? "Selected " + selected : "Choose a date"}</CardDescription></div><Button size="icon" variant="outline" disabled={!nextMonth} onClick={()=>move(1)} aria-label="Next month"><ChevronRight className="h-4 w-4"/></Button></div></CardHeader><CardContent><div className="grid grid-cols-7 gap-1">{weekdays.map(day=><div key={day} className="p-2 text-center text-xs font-semibold text-muted-foreground">{day}</div>)}{cells.map(cell=><button key={cell.key} type="button" disabled={!cell.isoDate} onClick={()=>cell.isoDate&&setSelected(cell.isoDate)} className={"aspect-square rounded-lg border text-sm disabled:border-transparent "+(selected===cell.isoDate?"border-primary bg-primary text-primary-foreground":"hover:bg-muted")}>{cell.day??""}</button>)}</div></CardContent></Card>
