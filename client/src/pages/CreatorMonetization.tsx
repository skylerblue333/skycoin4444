...
const MONETIZATION_FEATURES = [
  {
    ...
    title: "Subscriptions",
    desc: "Monthly recurring revenue from fans",
    href: "/subscriptions",
  ...
  },
  {
    ...
    desc: "One-time payments from supporters",
    href: "/tips",
  ...
  },
  {
    ...
    desc: "Brand partnerships and ad revenue",
  ...
  },
  {
    ...
    title: "Stream Gifts",
    desc: "Real-time gifts during live streams",
    href: "/streaming",
  ...
  },
  {
    ...
    desc: "Earn from referrals and partnerships",
    href: "/affiliate",
...
export default function CreatorMonetization() {
  ...
  const { data: earnings } = trpc.creator.earnings.useQuery(undefined, {
    enabled: isAuthenticated,
  });
  const { data: milestones } = trpc.creatorGrowth.getMilestones.useQuery(
    undefined,
    { enabled: isAuthenticated }
  );
  const { data: advice } = trpc.creatorGrowth.getGrowthAdvice.useQuery(
    undefined,
    { enabled: isAuthenticated }
  );
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-muted-foreground mb-6">
            Turn your content into income. Multiple revenue streams, all in one
            place.
          </p>
  ...
  return (
    <div className="min-h-screen">
      <PageHeader
        ...
        subtitle="Multiple revenue streams · Real earnings · Growth tools"
      />
      <div className="container py-6 max-w-6xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            {
              label: "Tips",
              value: `$${tipRevenue.toFixed(2)}`,
              icon: <Heart className="w-4 h-4" />,
              color: "text-pink-400",
            },
            {
              label: "Active Subs",
              value: String(earnings?.subscriptions || 0),
                  ...
                  <div className="rounded-xl bg-white/5 border border-white/10 p-5 hover:bg-white/8 transition-all cursor-pointer group">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Badge
                        ...
                        variant={f.badge === "Active" ? "secondary" : "outline"}
                      >
                        {f.badge}
                      </Badge>
                    </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {MONETIZATION_FEATURES.map(f => (
                    ...
                    >
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {((f.revenue / totalRevenue) * 100).toFixed(1)}% of
                        total
                      </div>
                    </div>
                  ))}
              ...
              <div className="rounded-xl bg-white/5 border border-white/10 p-5">
                ...
                {!milestones || (milestones as any[]).length === 0 ? (
                  <p className="text-muted-foreground text-sm">
                    No milestones yet. Keep creating!
                  </p>
                ) : (
                  <div className="space-y-3">
                    {(milestones as any[])
                      ...
                      .map((m: any, i: number) => (
                        <div key={i} className="flex items-center gap-3">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${m.achieved ? "bg-yellow-500/20 text-yellow-400" : "bg-white/5 text-muted-foreground"}`}
                          >
                            {m.achieved ? "✓" : i + 1}
                          </div>
              ...
              <div className="rounded-xl bg-white/5 border border-white/10 p-5">
                ...
                {!advice ? (
                  ...
                  <div className="space-y-3">
                    {(
                      (advice as any).tips || [
                        (advice as any).advice ||
                          "Keep creating consistently and engage with your audience daily.",
                      ]
                    )
                      .slice(0, 4)
