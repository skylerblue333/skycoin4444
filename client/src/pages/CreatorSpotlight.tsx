...
const FEATURED_CREATORS = [
  {
    ...
    views: 4200000,
    earnings: 28400,
    categories: ["Tech", "Crypto", "AI"],
    isVerified: true,
    isLive: true,
    liveViewers: 847,
    gradient: "from-purple-600 to-pink-600",
    recentContent: [
      "New AI trading strategy drops tonight 🔥",
      "Web3 tutorial series — Part 12",
      "SKYCOIN4444 deep dive",
    ],
  },
  {
    id: "c2",
    ...
    recentContent: [
      ...
      "NFT market analysis Q2 2026",
      "How I made 10x on SKYCOIN",
    ],
  },
  {
    id: "c3",
    ...
    views: 8900000,
    earnings: 14800,
    categories: ["Gaming", "Streaming", "Entertainment"],
    isVerified: true,
    isLive: true,
    liveViewers: 2341,
    gradient: "from-green-600 to-teal-600",
    recentContent: [
      "Shadow Arena world record attempt",
      "Crypto gaming marathon — 12hrs",
      "New merch drop this Friday",
    ],
  },
  {
    id: "c4",
    ...
    recentContent: [
      ...
      "AI art tutorial — Stable Diffusion",
      "Collab with @skylerdev",
    ],
  },
  {
    id: "c5",
    ...
    views: 3400000,
    earnings: 11200,
    categories: ["Tech", "Philosophy", "Writing"],
    isVerified: true,
    isLive: false,
    liveViewers: 0,
  ...
  },
  {
    ...
    views: 890000,
    earnings: 5400,
    categories: ["Fitness", "Crypto", "Lifestyle"],
    isVerified: false,
    isLive: false,
    liveViewers: 0,
    gradient: "from-yellow-600 to-orange-600",
    recentContent: [
      "30-day move-to-earn challenge",
      "Best crypto fitness apps 2026",
      "Morning routine + portfolio review",
    ],
  },
];
...
function CreatorCard({ creator }: { creator: (typeof FEATURED_CREATORS)[0] }) {
  ...
  const handleFollow = () => {
    ...
    setFollowed(f => !f);
    toast.success(
      followed ? `Unfollowed ${creator.name}` : `Following ${creator.name}!`
    );
  };
  ...
  const handleSubscribe = () => {
    ...
    setSubscribed(s => !s);
    toast.success(
      subscribed ? "Unsubscribed" : `Subscribed to ${creator.name}! 🎉`
    );
  };
        ...
        {creator.isLive && (
          <div className="absolute top-3 right-3 flex items-center gap-1.5 px-2.5 py-1 bg-red-500 rounded-full">
            ...
            <span className="text-[10px] text-white/80">
              {creator.liveViewers.toLocaleString()}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="px-4 pb-4">
        <div className="flex items-end justify-between -mt-8 mb-3">
          <Avatar className="w-16 h-16 border-4 border-card text-xl font-bold">
            <AvatarFallback
              className={`bg-gradient-to-br ${creator.gradient} text-white text-xl font-bold`}
            >
              {creator.avatar}
            </AvatarFallback>
          </Avatar>
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {creator.categories.map(c => (
            <Badge key={c} variant="secondary" className="text-[10px] px-2">
              {c}
            </Badge>
          ))}
        </div>
...
export default function CreatorSpotlight() {
  ...
  const filtered = FEATURED_CREATORS.filter(c => {
    ...
    const matchFilter =
      filter === "all" ||
      (filter === "live" && c.isLive) ||
      (filter === "verified" && c.isVerified) ||
      c.tier.toLowerCase() === filter;
    ...
    <div className="container max-w-7xl mx-auto py-6 px-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Discover top creators, subscribe for exclusive content, and support
            your favorites
          </p>
        </div>
        ...
        <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-4 mb-6">
          ...
          >
            {liveCreators.map(c => (
              <div
                key={c.id}
                className="flex-shrink-0 flex items-center gap-2 px-3 py-2 bg-card border border-border/50 rounded-xl hover:border-red-500/30 transition-all cursor-pointer"
              >
                <Avatar className="w-8 h-8">
                  <AvatarFallback
                    className={`bg-gradient-to-br ${c.gradient} text-white text-sm font-bold`}
                  >
                    {c.avatar}
                  </AvatarFallback>
                </Avatar>
                <div>
                  ...
                  <div className="flex items-center gap-1 text-[10px] text-red-400">
                    ...
                    {c.liveViewers.toLocaleString()} watching
                  </div>
                </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          >
            <div
              className={`w-9 h-9 rounded-xl bg-secondary flex items-center justify-center ${s.color}`}
            >
      ...
      <div className="mt-12 bg-gradient-to-r from-purple-600/10 to-pink-600/10 border border-purple-500/20 rounded-2xl p-8 text-center">
        ...
        <p className="text-muted-foreground mb-6 max-w-md mx-auto">
          Join 12,400+ creators earning on SKYCOIN4444. Subscriptions, tips, PPV
          content, and live streaming — all in one platform.
        </p>
