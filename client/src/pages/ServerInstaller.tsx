...
const INSTALL_STEPS = [
  {
    id: 1,
    title: "Prerequisites",
    desc: "Docker 24+, Node 20+, 4GB RAM",
    done: true,
  },
  {
    id: 2,
    title: "Clone Repository",
    desc: "git clone shadowchat",
    done: true,
  },
  {
    id: 3,
    title: "Configure Environment",
    desc: "Set API keys and database URL",
    done: false,
  },
  {
    id: 4,
  ...
  { id: 5, title: "Start Services", desc: "docker-compose up -d", done: false },
  {
    ...
    desc: "Auto-configure Let's Encrypt",
    done: false,
  },
  {
    id: 7,
    title: "Health Check",
    desc: "Verify all services running",
    done: false,
  },
];
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="text-center mb-8 p-6 rounded-2xl bg-gradient-to-br from-green-900/20 to-blue-900/20 border border-green-500/20">
          ...
          <h2 className="text-2xl font-bold mb-2">
            Deploy ShadowChat on Your Server
          </h2>
          <p className="text-gray-400 max-w-lg mx-auto text-sm">
            Full self-hosted deployment with Docker Compose. Own your data,
            customize everything, run on any cloud or bare metal.
          </p>
          <div className="flex items-center justify-center gap-4 mt-4 text-xs text-gray-500">
            <span className="flex items-center gap-1">
              <Shield className="w-3 h-3 text-green-400" /> Full data ownership
            </span>
            <span className="flex items-center gap-1">
              <Zap className="w-3 h-3 text-yellow-400" /> 5-min setup
            </span>
            <span className="flex items-center gap-1">
              <Globe className="w-3 h-3 text-blue-400" /> Any cloud provider
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    {currentStep === 3 && generatedFiles && (
                      <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-3">
                        ...
                        <p className="text-xs text-gray-400">
                          Your custom config files are ready. Download each file
                          and place them in your server directory.
                        </p>
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <Button
                            ...
                            className="border-white/10 text-xs"
                            onClick={() =>
                              downloadFile(generatedFiles.envFile, ".env")
                            }
                          >
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="border-white/10 text-xs"
                        onClick={() =>
                          copyToClipboard(DOCKER_COMPOSE, "docker")
                        }
                      >
                        {copied === "docker" ? (
                      ...
                      <Button
                        ...
                        className="border-white/10 text-xs"
                        onClick={() =>
                          downloadFile(DOCKER_COMPOSE, "docker-compose.yml")
                        }
                      >
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        className="border-white/10 text-xs"
                        onClick={() =>
                          copyToClipboard(QUICK_INSTALL_SCRIPT, "script")
                        }
                      >
                        {copied === "script" ? (
                          <CheckCircle className="w-3 h-3 text-green-400" />
                      ...
                      <Button
                        ...
                        className="border-white/10 text-xs"
                        onClick={() =>
                          downloadFile(QUICK_INSTALL_SCRIPT, "install.sh")
                        }
                      >
                        <Download className="w-3 h-3 mr-1" /> Download
                      </Button>
                  ...
                  <CardContent>
                    <div className="flex items-start gap-2 p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20 mb-4">
                      ...
                      <p className="text-xs text-yellow-300">
                        Review the script before running. Run on a clean Ubuntu
                        22.04+ server with root access.
                      </p>
                    </div>
                    <pre className="bg-black/60 rounded-xl p-4 text-xs text-green-300 font-mono overflow-x-auto whitespace-pre-wrap border border-white/5">
                  ...
                  <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        className="border-white/10 text-xs"
                        onClick={() =>
                          downloadFile(ENV_TEMPLATE, ".env.example")
                        }
                      >
                        <Download className="w-3 h-3 mr-1" /> Download
                      </Button>
          ...
          <div className="space-y-4">
            <Card className="bg-black/40 border-white/10">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
                  <Server className="w-4 h-4 text-green-400" /> System
                  Requirements
                </CardTitle>
              </CardHeader>
            ...
            <Card className="bg-black/40 border-white/10">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
                  <Database className="w-4 h-4 text-purple-400" /> Quick
                  Commands
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {[
                  ...
                  {
                    cmd: "docker-compose exec app pnpm db:push",
                    desc: "Run migrations",
                  },
                  ...
                  <div key={item.cmd} className="space-y-0.5">
                    <div className="flex items-center justify-between">
                      <code className="text-xs text-green-300 font-mono bg-black/40 px-2 py-0.5 rounded">
                        {item.cmd}
                      </code>
                      <button
