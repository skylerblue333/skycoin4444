...
const PLATFORM_STATS = [
  {
    label: "Total Users",
    value: "Unavailable",
    change: "Verified analytics required",
    icon: Users,
    color: "text-blue-400",
    bg: "bg-blue-500/10",
  },
  {
    label: "SKY444 Price",
    value: "Unavailable",
    change: "Live market data required",
    icon: Coins,
    color: "text-amber-400",
    bg: "bg-amber-500/10",
  },
  {
    label: "Total Staked",
    value: "Unavailable",
    change: "Verified staking data required",
    icon: Shield,
    color: "text-emerald-400",
    bg: "bg-emerald-500/10",
  },
  {
    label: "Daily Volume",
    value: "Unavailable",
    change: "Verified volume data required",
    icon: Activity,
    color: "text-purple-400",
    bg: "bg-purple-500/10",
  },
      ...
      <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-slate-400 mb-8">
            The world's most advanced Web3 social ecosystem. AI · DeFi · Gaming
            · Creator Economy.
          </p>
          <a
            href={getLoginUrl()}
            className="inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-bold px-8 py-4 rounded-2xl text-lg hover:opacity-90 transition-opacity"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {isOwner && (
          <div className="rounded-2xl border border-slate-800/60 bg-gradient-to-br from-slate-900/80 to-slate-950/80 overflow-hidden">
            <div className="relative h-32 bg-gradient-to-r from-cyan-900/40 via-purple-900/40 to-slate-900/40">
              <img
                src={SKYLER_PHOTOS.innovativeIT}
                alt="Innovative IT Resolutions"
            ...
            <div className="px-6 pb-6 -mt-12 relative">
              <div className="flex items-end gap-4 mb-4">
                <div className="relative">
                  <img
                    src={SKYLER_PHOTOS.family}
                    alt="Skyler"
                    className="w-20 h-20 rounded-2xl border-4 border-slate-950 object-cover"
                  />
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-slate-400">
                    Software Engineer · Innovative IT Resolutions LLC
                  </p>
                  <p className="text-xs text-cyan-400 mt-0.5">
                    @skycoin4444 · Platform Founder
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  <Link href="/creator-studio">
                    <Button
                      size="sm"
                      className="bg-gradient-to-r from-cyan-500 to-purple-600 text-white text-xs border-0"
                    >
                      Creator Studio
                    </Button>
                  </Link>
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {[
                  ...
                  SKYLER_PHOTOS.chess,
                  SKYLER_PHOTOS.nikes,
                  SKYLER_PHOTOS.family,
                ].map((src, i) => (
                  <img
                    key={i}
                    src={src}
                    alt=""
                    className="w-16 h-16 rounded-xl object-cover shrink-0 border border-slate-700/50 hover:border-cyan-500/50 transition-colors cursor-pointer"
                  />
                ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {PLATFORM_STATS.map(s => (
            <div
              key={s.label}
              className={`rounded-2xl border border-slate-800/60 p-4 ${s.bg} relative overflow-hidden`}
            >
              ...
              <div className="mt-1 text-xs text-slate-500">{s.change}</div>
            </div>
          ))}
        </div>
        ...
        <div>
          ...
          <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/40 p-6 text-center text-sm text-slate-500">
            Portfolio balances, prices, and performance are unavailable until
            verified wallet and market-data integrations are connected.
          </div>
        </div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-12 gap-2">
            {QUICK_LINKS.map(l => (
              <Link key={l.name} href={l.href}>
                <div
                  className={`rounded-xl border border-slate-800/60 ${l.bg} p-3 flex flex-col items-center gap-2 hover:border-slate-700 transition-all cursor-pointer group`}
                >
                  <l.icon
                    className={`h-5 w-5 ${l.color} group-hover:scale-110 transition-transform`}
                  />
        ...
        <div className="rounded-2xl border border-slate-700/40 overflow-hidden relative">
          <img
            src={SKYLER_PHOTOS.innovativeIT}
            alt="Innovative IT Resolutions LLC"
