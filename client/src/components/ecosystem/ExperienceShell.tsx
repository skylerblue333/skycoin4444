...
import { BarChart3, Gamepad2, GraduationCap, Heart, Home, Radio, Search, ShoppingBag, Sparkles, Wallet } from "lucide-react";
...
const ACCENTS: Record<ExperienceAccent, { pill: string; icon: string; glow: string }> = {
  violet: { pill: "bg-violet-600 text-white shadow-violet-500/20", icon: "bg-violet-100 text-violet-700", glow: "from-violet-500/10 via-transparent to-transparent" },
  pink: { pill: "bg-pink-600 text-white shadow-pink-500/20", icon: "bg-pink-100 text-pink-700", glow: "from-pink-500/10 via-transparent to-transparent" },
  indigo: { pill: "bg-indigo-600 text-white shadow-indigo-500/20", icon: "bg-indigo-100 text-indigo-700", glow: "from-indigo-500/10 via-transparent to-transparent" },
...
export function ExperienceShell({ title, subtitle, icon: Icon, accent, badge, actions, children }: ExperienceShellProps) {
  ...
  const nav = (compact = false) => NAV_ITEMS.map((item) => {
        ...
        <main className="relative min-w-0 flex-1 overflow-hidden">
          ...
          <header className="relative z-10 border-b border-slate-200/80 bg-white/90 px-4 py-4 backdrop-blur md:px-7">
            <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4">
              <div className="flex min-w-0 items-center gap-3"><div className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${palette.icon}`}><Icon className="h-5 w-5" aria-hidden="true" /></div><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-xl font-bold tracking-tight md:text-2xl">{title}</h1>{badge ? <span className="rounded-full border border-slate-200 bg-white px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500">{badge}</span> : null}</div><p className="mt-0.5 truncate text-xs text-slate-500 md:text-sm">{subtitle}</p></div></div>
