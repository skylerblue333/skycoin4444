...
**Latest checkpoint:** `4b71993`  
**Release label:** **Code-green launch candidate; GA not authorized**

## Honest percentage

The current **directional launch-readiness estimate is 38%**. This is not a marketing claim, uptime metric, or formal certification. It is a weighted engineering score that prevents a green local build from hiding unverified infrastructure and critical workflows.

| Area | Weight | Current score | Basis |
|---|---:|---:|---|
| Code correctness and build health | 25% | 23% | Zero strict TypeScript diagnostics, passing production build, clean diff, and four passing tests. The score is not full because test depth remains limited. |
| Frontend inventory and navigation scope | 10% | 10% | All 1,057 page files and 1,061 route lines were inventoried. Inventory completeness is not feature completeness. |
| Frontend feature truthfulness and usability | 25% | 10% | 121 pages are explicitly bounded as unavailable, while 675 contain mock/demo/placeholder-style markers requiring classification or repair. Portfolio, exchange, mining, marketplace, progress tracking, and dashboard boundaries were improved. |
| Critical workflow verification | 15% | 3% | Logout and truthful-boundary regressions pass. Registration, login, profile operations, wallet ledger, education, admin authorization, and representative integrations still need real integration or staging tests. |
...
| Operational/security acceptance | 5% | 2% | Repository-level checks and truthful-state controls exist, but production secrets, least privilege, alerting, recovery objectives, and owner acceptance remain unverified. |
| **Total** | **100%** | **38%** | **Directional launch-readiness estimate** |
...
The strict TypeScript baseline is green at zero diagnostics. Production builds, the available automated suite, and diff hygiene pass. The test suite now contains four passing tests across two files: the existing logout test and three truthful-boundary regressions.
...
| Explicit unavailable boundaries | 121 | These pages are honest but intentionally not active. |
| Mock/demo/placeholder markers | 675 | These require manual classification and likely repair; the count is a screening signal, not a defect count. |
