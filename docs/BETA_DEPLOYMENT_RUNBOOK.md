...
A release owner connects the GitHub repository to Render, creates the service from `render.yaml`, configures the environment variables, and confirms the service uses the intended region and access policy. Before starting the application, the owner applies the migrations in order against the beta database. For an existing managed beta database, the Language Exchange upgrade uses the reviewed additive command `BETA_DB_MIGRATION_CONFIRM=LANGUAGE_EXCHANGE_V1 pnpm beta:db:migrate:language-exchange`; the script verifies columns, index, and the `users` foreign key before reporting success. The owner then performs a manual deploy from the exact green `main` commit.
