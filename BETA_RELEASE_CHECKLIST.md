...
| Session JWT expiration and cookie Max-Age use the same bounded configured lifetime, and the stateless-revocation limitation is documented.              | Deployment config plus controlled issued-session/cookie evidence. | Identity/security owner         | [ ]      |
| Session signing-key rotation, when active, uses one distinct verification-only previous key and has a recorded removal date/window.                      | Secret-manager configuration review plus active/previous token verification evidence; no secret values recorded. | Identity/security owner | [ ] |
| Oversized or malformed session JWTs are rejected before signature verification and normal issued tokens remain within the bound.                         | Focused token-input tests plus exact-head CI evidence. | Identity/security owner | [ ] |
| Production browser sessions use the host-prefixed session cookie, omit Domain, and legacy unprefixed cookies are not trusted for authentication.          | Controlled Set-Cookie metadata plus login/logout migration evidence; token value redacted. | Identity/security owner | [ ] |
...
| Canonical request IDs are server-generated and caller IDs remain separately labeled untrusted correlation metadata.                                     | Controlled request with supplied `X-Request-ID` plus application log/response evidence. | Engineering/security owner | [ ] |
| Operational error logging and durable outbox failure text are checked for credential/token redaction.                                                   | Controlled sanitized-error tests plus CI evidence. | Engineering/security owner | [ ] |
...
| HTTP header-count and connection limits are explicitly configured and match the deployed runtime-state diagnostic.                                       | Deployment config plus `/api/runtime/state` evidence. | Operations owner | [ ] |
