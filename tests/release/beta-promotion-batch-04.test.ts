...
const pagePaths = {
  formatter: "client/src/pages/CodeFormatter.tsx",
...
describe("beta forms utility core", () => {
  it("counts and transforms plain text deterministically", () => {
  ...
  it("formats JSON without executing code and rejects malformed JSON", () => {
    expect(formatCode('{"b":2,"a":1}', "json-pretty")).toContain("\n");
    expect(formatCode('{ "b": 2 }', "json-minify")).toBe('{"b":2}');
    expect(formatCode("x  \n\n\n y\t ", "normalize-whitespace")).toBe(
      "x\n\n y"
    );
    expect(() => formatCode("{bad}", "json-pretty")).toThrow();
  });
...
describe("beta forms utility pages", () => {
  it("replaces every generated shell with meaningful interactions", () => {
    ...
    expect(sources.formatter).toContain("formatCode");
  ...
  it("states explicit local/provider/security boundaries", () => {
    expect(sources.formatter).toMatch(
      /not a JavaScript\/TypeScript compiler, linter, sandbox, or Prettier replacement/i
    );
