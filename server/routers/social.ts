...
export const createPostProcedure = protectedProcedure
  .input(z.object({ content: z.string().trim().min(1).max(255), media: z.string().trim().max(255).nullable().optional() }))
  .mutation(async ({ ctx, input }) => {
    ...
    return { id, userId: ctx.user.id, content: input.content, media: input.media ?? null };
  });
...
export const listCommentsProcedure = publicProcedure
  .input(postIdInput.extend({ limit: z.number().int().min(1).max(100).default(30) }))
  .query(async ({ input }) => {
    const rows = await db
      .select({
        id: comments.id,
    ...
    return rows.map(row => ({
      ...
      author: row.authorId
        ? { id: row.authorId, username: row.authorUsername, name: row.authorName, avatar: row.authorAvatar, verified: row.authorVerified ?? false }
        : null,
    }));
  });

export const addCommentProcedure = protectedProcedure
  .input(postIdInput.extend({ content: z.string().trim().min(1).max(255) }))
  .mutation(async ({ ctx, input }) => {
    const post = await db.query.posts.findFirst({ where: eq(posts.id, input.postId) });
    if (!post) throw new TRPCError({ code: "NOT_FOUND", message: "Post not found" });

    const id = randomUUID();
    await db.insert(comments).values({ id, postId: input.postId, userId: ctx.user.id, content: input.content });
    await db.update(posts).set({ comments: sql`COALESCE(${posts.comments}, 0) + 1` }).where(eq(posts.id, input.postId));
    return { id, postId: input.postId, userId: ctx.user.id, content: input.content };
  });
...
export const likePostProcedure = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    await db.insert(likes).values({ id: randomUUID(), postId: input.postId, userId: ctx.user.id });
    await db.update(posts).set({ likes: sql`COALESCE(${posts.likes}, 0) + 1` }).where(eq(posts.id, input.postId));
...
export const unlikePostProcedure = protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    await db.delete(likes).where(and(eq(likes.postId, input.postId), eq(likes.userId, ctx.user.id)));
    await db.update(posts).set({ likes: sql`GREATEST(COALESCE(${posts.likes}, 0) - 1, 0)` }).where(eq(posts.id, input.postId));
