...
import { arcadeGameProgress, betaFeedback, courseProgress, creatorEvidenceDrafts, discoveryBookmarks, posts, privacyRequests, searchHistory } from "../../drizzle/schema";
import { db } from "../db";
...
export const activityEvidenceRouter = router({
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    const events = [
      ...
      ...gameRows.map(row => ({ id: `game:${row.id}`, type: "game_progress_synced", label: "Synced arcade progress", detail: `${row.gameId} · ${row.plays} recorded runs · best score ${row.bestScore}`, createdAt: row.updatedAt })),
      ...
      ...privacyRows.map(row => ({ id: `privacy:${row.id}`, type: "privacy_request", label: "Recorded a privacy request", detail: `${row.action} · ${row.status}`, createdAt: row.requestedAt })),
    ];
