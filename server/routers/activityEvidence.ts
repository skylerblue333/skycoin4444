  ...
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [
      ...
      db
        .select({
          id: betaFeedback.id,
          route: betaFeedback.route,
          summary: betaFeedback.summary,
      ...
      db
        .select({
          id: discoveryBookmarks.id,
          title: discoveryBookmarks.title,
          targetKind: discoveryBookmarks.targetKind,
          createdAt: discoveryBookmarks.createdAt,
        })
        .from(discoveryBookmarks)
        .where(eq(discoveryBookmarks.userId, userId))
        .orderBy(desc(discoveryBookmarks.createdAt))
        .limit(25),
      db
        .select({
          id: searchHistory.id,
      ...
      db
        .select({
          id: creatorEvidenceDrafts.id,
          title: creatorEvidenceDrafts.title,
          status: creatorEvidenceDrafts.status,
          updatedAt: creatorEvidenceDrafts.updatedAt,
        })
        .from(creatorEvidenceDrafts)
        .where(eq(creatorEvidenceDrafts.userId, userId))
        .orderBy(desc(creatorEvidenceDrafts.updatedAt))
        .limit(25),
      db
        .select({
          id: privacyRequests.id,
          action: privacyRequests.action,
    ...
    const gameEvents = gameRows.flatMap(row => {
      const session = decodeBetaGameSession(row.metadata);
      if (!session) return [];
      return [
        {
          id: `game:${row.id}`,
          type: "game_run_saved",
          label: `Saved a ${betaGameLabel(session.gameId)} run`,
          detail: `${session.score} points · ${session.sparks} Sparks · ${session.mode}`,
    ...
    const events = [
      ...postRows.map(row => ({
        ...
        detail: row.content || "Post content is not available",
        createdAt: row.createdAt ?? new Date(0),
      })),
      ...courseRows.map(row => ({
        id: `lesson:${row.id}`,
        ...
        detail: `${row.courseId} · ${row.lessonId}`,
        createdAt: row.completedAt,
      })),
      ...feedbackRows.map(row => ({
        id: `feedback:${row.id}`,
        type: "feedback_submitted",
        label: "Submitted beta feedback",
        detail: `${row.route} · ${row.summary} · ${row.status}`,
        createdAt: row.createdAt ?? new Date(0),
      })),
      ...bookmarkRows.map(row => ({
        id: `bookmark:${row.id}`,
        type: "bookmark_saved",
        label: "Saved a discovery reference",
        detail: `${row.targetKind} · ${row.title}`,
        createdAt: row.createdAt,
      })),
      ...historyRows.map(row => ({
        id: `search:${row.id}`,
        type: "search_recorded",
        label: "Recorded a search",
        detail: row.query,
        createdAt: row.createdAt,
      })),
      ...creatorRows.map(row => ({
        id: `creator:${row.id}`,
        type: "creator_draft_updated",
        label: "Updated a creator brief",
      ...
      ...privacyRows.map(row => ({
        ...
        label: "Recorded a privacy request",
        detail: `${row.action} · ${row.status}`,
