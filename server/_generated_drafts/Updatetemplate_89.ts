...
export const templateRouter = router({
  updateTemplate: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!name && !content) {
        return { success: false, message: 'No update data provided. Either name or content must be present.' };
      }
      ...
      try {
        ...
        return { success: false, message: 'An unexpected error occurred while updating the template.' };
      }
    }),
});