...
if (!databaseUrl) {
  throw new Error(
    "DATABASE_URL is required to start the server; refusing to use an implicit or mock database."
  );
}
    ...
    for (const tokenSymbol of defaultTokens) {
      const existingBalance = await db.query.tokenBalances.findFirst({
        where: (tokenBalances, { eq: eqOp, and: andOp }) =>
          andOp(
            eqOp(tokenBalances.userId, userId),
            eqOp(tokenBalances.tokenSymbol, tokenSymbol)
          ),
      });
      if (!existingBalance) {
