...
const orderStatuses = new Set<string>(["draft", "placed", "cancelled", "fulfilled"]);
...
export function validateOrder(order: Order): string[] {
  ...
  for (const line of order.lines) {
    ...
    if (!validQuantity) errors.push(`quantity must be positive for ${line.sku || "<empty>"}`);
    ...
    if (line.sku.trim() && validQuantity && !overflowedSkus.has(line.sku)) {
      ...
      if (!Number.isSafeInteger(combined)) {
        errors.push(`total quantity exceeds safe integer range for ${line.sku}`);
...
export function validateAvailability(availability: readonly InventoryAvailability[]): string[] {
  const errors: string[] = [];
  ...
  for (const item of availability) {
    ...
    if (!Number.isSafeInteger(item.available) || item.available < 0) {
      errors.push(`availability must be a non-negative safe integer for ${item.sku || "<empty>"}`);
    }
    if (seen.has(item.sku)) errors.push(`duplicate availability sku: ${item.sku}`);
    seen.add(item.sku);
  }
...
export function evaluatePlacement(order: Order, availability: readonly InventoryAvailability[]): PlacementDecision {
  ...
  if (availabilityErrors.length > 0) {
    return { accepted: false, reason: availabilityErrors.join("; "), totalMinor, shortages: [] };
  }
  ...
  for (const line of order.lines) {
    ...
    // validateOrder already proves this aggregate is safe; keep the guard here
    // so future callers/refactors cannot silently reintroduce unsafe arithmetic.
    if (!Number.isSafeInteger(combined)) {
      return {
        accepted: false,
...
export function transitionOrder(order: Order, next: OrderStatus): Order {
  ...
  if (!isOrderStatus(order.status)) throw new Error(`invalid current order status: ${String(order.status)}`);
  if (!isOrderStatus(next)) throw new Error(`invalid target order status: ${String(next)}`);
