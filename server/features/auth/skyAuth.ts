...
export type AuthDecision =
  ...
  | {
      ok: false;
      code: "invalid_claims" | "expired" | "not_yet_valid";
      reason: string;
    };
 ...
 */
export function authenticateVerifiedSession(
  input: unknown,
  ...
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    return {
      ok: false,
      code: "invalid_claims",
      reason: "claims must be an object",
    };
  }
  ...
  if (!isSafeIdentifier(subject) || !isSafeIdentifier(sessionId)) {
    return {
      ok: false,
      code: "invalid_claims",
      reason: "subject and sessionId must be safe identifiers",
    };
  }

  if (
    !isFiniteInteger(issuedAtMs) ||
    !isFiniteInteger(expiresAtMs) ||
    expiresAtMs <= issuedAtMs
  ) {
    return {
      ok: false,
      code: "invalid_claims",
      reason: "session timestamps are invalid",
    };
  }
  ...
  if (expiresAtMs - issuedAtMs > MAX_SESSION_LIFETIME_MS) {
    return {
      ok: false,
      code: "invalid_claims",
      reason: "session lifetime exceeds policy maximum",
    };
  }
  ...
  if (typeof authMethod !== "string" || !AUTH_METHODS.has(authMethod)) {
    return {
      ok: false,
      code: "invalid_claims",
      reason: "authMethod is unsupported",
    };
  }
  ...
  if (issuedAtMs > nowMs + MAX_CLOCK_SKEW_MS) {
    return {
      ok: false,
      code: "not_yet_valid",
      reason: "session was issued too far in the future",
    };
  }
